...
function start({ message, chatId, sessionId, cwd, permissionMode, allowedTools, addDirs, model } = {}) {
  ...
  const run = { proc, requestId, chat, startedAt: Date.now(), buf: '' };
  ...
  proc.stderr && proc.stderr.on('data', (chunk) => {
    // Live-only, unbuffered (no seq): chatty and worthless in a replay.
  ...
  proc.on('exit', (code, signal) => {
    ...
    settle(signal ? { type: 'aborted' } : (code === 0 ? { type: 'done', sessionId: chat.sessionId } : { type: 'error', error: 'claude exited with code ' + code }));
  });
...
module.exports = {
  ...
  // exported for tests
  splitLines, sessionIdOf, previewOf, encodeCwd, createChat, pushEvent, sanitizeAddDirs,
};
