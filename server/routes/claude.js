...
// async: start() may accept the folder-trust dialog before spawning.
router.post('/chat', async (req, res) => {
  try {
    const { message, chatId, sessionId, cwd, permissionMode, allowedTools, addDirs, model } = req.body || {};
    const r = await interactive.start({ message, chatId, sessionId, cwd, permissionMode, allowedTools, addDirs, model });
    res.json(r);
