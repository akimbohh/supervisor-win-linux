...
// Kept dependency-free; loaded as a global on window.
...
(function () {
  ...
  // Path helpers (work for both / and \ separators).
  // All defensively coerce to string so callers can't crash us by passing
  // an object/number — we just return '' instead.
  function basename(p) {
    if (p == null) return '';
  ...
  function emptyState({ icon: ic, title, body, action }) {
    ...
    if (body) {
      const p = el('p', null);
      // Allow line-breaks in body — useful for showing stack traces on error.
  ...
  function skeleton(rows = 6) {
    ...
    for (let i = 0; i < rows; i++) {
      root.appendChild(el('div', { class: 'skel', style: { height: '36px', borderRadius: '8px' } }));
    }
    return root;
  }

  // Confirmation modal helper. Uses an idempotent settle() so multiple paths
  // to dismissal can't race (e.g. action handler + onClose firing).
  ...
  async function prompt({ title = 'Enter value', label = '', initial = '', placeholder = '', confirmText = 'OK' } = {}) {
    return new Promise((resolve) => {
      let done = false;
      const settle = (v) => { if (done) return; done = true; resolve(v); };
      const input = el('input', { class: 'input', value: initial, placeholder });
      ...
      let handle = null;
      const submit = () => { const v = input.value; settle(v); handle && handle.close(); };
  ...
  // Pull-to-refresh — call attachPullToRefresh(scrollContainer, onRefresh).
