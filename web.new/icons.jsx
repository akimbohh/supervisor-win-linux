/* Inline SVG icons (Lucide-derived). 1.7px stroke. 24x24 viewBox.
 *
 * Two parallel APIs share a single PATHS table:
 *   - window.icon(name, { size, className, strokeWidth }) → SVG HTML string
 *     (used by imperative DOM code: modal, toast, sheet, util)
 *   - <Icon name="folder" size={16}/>  (React component used by the views)
 *
 * Kebab-case names (chevron-right) and camelCase aliases (chevronRight) both
 * resolve to the same path so existing view code keeps working.
 */
...
(function () {
  const PATHS = {
    ...
    git: '<circle cx="12" cy="6" r="3"/><circle cx="6" cy="18" r="3"/><circle cx="18" cy="18" r="3"/><path d="M9 6h6a3 3 0 0 1 3 3v6"/><path d="M12 9v6"/>',
    code: '<polyline points="16 18 22 12 16 6"/><polyline points="8 6 2 12 8 18"/>',
    key: '<path d="M21 2l-2 2m-7.61 7.61a5.5 5.5 0 1 1-7.778 7.778 5.5 5.5 0 0 1 7.777-7.777zm0 0L15.5 7.5m0 0l3 3L22 7l-3-3m-3.5 3.5L19 4"/>',
  };

  /* camelCase aliases — keeps existing view code that imports `<Icon name="chevronRight"/>` working. */
  const ALIASES = {
  ...
  function resolve(name) {
    ...
    if (ALIASES[name]) return PATHS[ALIASES[name]] || '';
    return '';
  }

  /* HTML-string version — used by imperative DOM helpers (modal, toast, util). */
  ...
  window.ICONS = PATHS;
  console.info('[supervisor] icons loaded');

  /* React component — used by views authored in JSX. Single source of truth via PATHS. */
  window.Icon = function ({ name, size = 16, color, style, className, strokeWidth }) {
    const body = resolve(name);
    if (!body) return null;
    return (
      <svg
