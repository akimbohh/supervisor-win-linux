/* Stack-aware modal dialog — port of web/components/modal.js.
 * Multiple modals can stack; close() closes topmost; Esc closes topmost
 * dismissible. To race-proof callers, use an idempotent settle pattern:
 * onClose may fire even when an action handler also resolves your promise.
 *
 * Plain-JS in a .jsx file; lives outside the React tree in #modal-root.
 */
...
(function () {
  ...
  function close() { if (stack.length) popTop(); }
  ...
  function open({ title = '', content, actions = [], onClose = null, size = 'md', dismissible = true } = {}) {
    ...
    if (!root) {
      console.error('[modal] #modal-root missing — modal cannot mount');
      return { el: null, body: null, close: () => {} };
    }
  ...
  console.info('[supervisor] modal loaded');
})();
